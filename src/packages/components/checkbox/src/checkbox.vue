...
<div class="mt-checkbox" :class="{
      'mt-check-disabled': disabled,
      'mt-check-inline': isInline,
      [`mt-check-${state}`]: state,
    }">
    <label>
        <input type="checkbox" @change="handleChange" v-bind="$props" :disabled="isDisabled" v-model="getCurrenValue">
    ...
    computed: {
        getCurrenValue: {
            get() {
                    ...
                    return thisGroup ? thisGroup.currentGroupValue && thisGroup.currentGroupValue.indexOf(this.value) > -1 ? true : false : this.value
                },
                set(val) {
