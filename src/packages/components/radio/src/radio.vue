...
<div class="mt-radio" :class="{
      'mt-check-disabled': disabled,
      'mt-check-inline': isInline,
      [`mt-check-${state}`]: state,
    }">
    <label>
        <input type="radio" @change="handleChange" v-bind="$props" :disabled="isDisabled" :checked="getCurrenValue == val" :value="val" v-model="getCurrenValue">
